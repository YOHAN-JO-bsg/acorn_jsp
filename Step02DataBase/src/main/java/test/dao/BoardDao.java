...
public class BoardDao {
	...
	public int getCount() {
		...
		ResultSet rs = null;
		try {
			conn = new test.util.DbcpBean().getConn(); // DbcpBean() -> DBConnector()로 변경
			//실행할 sql문
			String sql = """
	...
	public List<BoardDto> selectPage(BoardDto dto){
		...
		try {
			...
			while (rs.next()) {
				...
				dto3.setNum(rs.getInt("num"));
				dto3.setWriter(rs.getString("writer"));
				dto3.setTitle(rs.getString("title"));
				dto3.setViewCount(rs.getInt("viewCount"));
				dto3.setCreatedAt(rs.getString("createdAt"));
