...
public class MemberListServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		pw.println("<body>");
		for(int i=0; i<5; i++) {
			pw.println("<p>hello"+i+"</p>"); // <p> 태그 닫는 부분도 </p> 로 수정했습니다.
		}
		...
		pw.println("<table border='1' cellpadding='8' cellspacing='0'>");
			...
			for(MemberDto tmp:list) {
				pw.println("<tr>"); // <-- 여는 tr 태그
					pw.println("<td>"+tmp.getNum()+"</td>"); // <-- 닫는 <td> 태그 수정
					pw.println("<td>"+tmp.getName()+"</td>"); // <-- 닫는 <td> 태그 수정
					pw.println("<td>"+tmp.getAddr()+"</td>"); // <-- 닫는 <td> 태그 수정
				pw.println("</tr>"); // <-- 닫는 tr 태그 추가
			}
			pw.println("</tbody>");
		pw.println("</table>");
		pw.println("</body>");
